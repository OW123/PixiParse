...
class Parser:
  ...
  def expr(self):
    ...
    if res.error:
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        "Expected 'VARIABLE', 'SI', 'FOR', 'WHILE', 'FUNCION', int, float, identifier, '+', '-', '(', '[' or 'NO'"
      ))
  ...
  def comp_expr(self):
    ...
    if res.error:
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        "Expected int, float, identifier, '+', '-', '(', '[', 'SI', 'FOR', 'WHILE', 'FUNCION' or 'NO'"
      ))
  ...
  def atom(self):
    ...
    return res.failure(InvalidSyntaxError(
      ...
      "Expected int, float, identifier, '+', '-', '(', '[', SI', 'FOR', 'WHILE', 'FUNCION'"
    ))
  ...
  def if_expr_cases(self, case_keyword):
    ...
    if not self.current_tok.matches(TT_KEYWORD, 'HAZ'):
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        f"Expected 'HAZ'"
      ))
...
class BuiltInFunction(BaseFunction):
  ...
  def execute_run(self, exec_ctx):
    ...
    if not isinstance(fn, String):
      return RTResult().failure(RTError(
        self.pos_start, self.pos_end, 
        "El argumento debe ser un texto",
        exec_ctx
      ))
    ...
    try:
      ...
      return RTResult().failure(RTError(
        self.pos_start, self.pos_end,
        f"No se pudo cargar el programa \"{fn}\"\n" + str(e),
        exec_ctx
      ))
    ...
    if error:
      return RTResult().failure(RTError(
        self.pos_start, self.pos_end,
        f"Fallo en la finalizacion de la ejecucion del programa \"{fn}\" \n" + error.as_string(),
        exec_ctx
      ))
...
global_symbol_table.set("CORRER", BuiltInFunction.run)
